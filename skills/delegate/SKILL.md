...
**Commit outcomes, not agent footsteps.** One builder run normally produces one implementation commit containing its code, task close, and changelog. A claim, body rewrite, review fix, proof rerun, or blocker note is not its own commit. During a multi-worker batch, leave same-domain tracker bookkeeping for the orchestrator's final integration commit when safe. Once every worker in the batch is idle, the orchestrator reads jj log and squashes same-domain fragments into the smallest reviewable set; never rewrite unrelated human/agent commits or reshape history while workers are active.
