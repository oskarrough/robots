...
The wrapper appends both contracts to the brief and verifies the resolved provider/model/thinking against the requested args before prompting (pi and claude). A `herdr_delegate_runtime_mismatch` at `stage: "verify"` means the worker started on the wrong runtime: close that pane and start over, don't brief it.
...
- Names: unique, ≤3 plain words, `[a-z][a-z0-9_-]{0,31}`; address agents by name, never pane ID. Check `agent list` first; your own pane is in it too.
...
`herdr-delegate` appends both contracts to every fresh brief, so don't paste them yourself; a warm `prompt` call doesn't repeat them:

> If blocked or you need a decision: stop, print one final line starting `ORCHESTRATOR: <question>`, and end your turn. Your settled pane is the message.

> End with a written report, not a tool call or silence. If you did nothing, say why.
...
| `blocked` | — | answer the `ORCHESTRATOR:` line; ask the human before answering a dialog |
